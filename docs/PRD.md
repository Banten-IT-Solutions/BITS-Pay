...
| ---------------------------------- | -------- | -------------------- |
| Workspace                          | 1        | 1                    |
| App per workspace                  | 1        | 3                    |
| Transaksi/bulan (sharing per akun) | 300      | 3.000                |
| Transaksi/hari (sharing per akun)  | 10       | 100                  |
| API Rate Limit                     | 10 req/s | 100 req/s            |
| Callback URL                       | ❌       | ✅                   |
| Webhook Retry                      | -        | 3x                   |
| Team Members                       | 1        | 5                    |

> Review manual & export laporan berlaku untuk **semua tier** (tidak dibatasi).
> Member tim yang sudah ada **tetap bisa akses** saat turun tier — hanya
> undangan baru yang diblokir.

> **Trial:** akun baru otomatis premium **14 hari** tanpa kartu kredit
> (`users.tier='premium'`, `tier_expires_at=+14 hari`, tanpa subscription).
> Habis → cron `expireAndDowngrade` menurunkan ke free.
>
> **Downgrade (trial habis / subscription expired / cancel / admin):**
> resource berlebih **dibekukan, bukan dihapus** — workspace & apps di luar jatah
> free (`is_active=0`, yang tertua dipertahankan). Efek: API key-nya 401,
> tampil ber-badge "Beku" di dashboard. Upgrade mengaktifkan kembali
> semuanya (`reactivateAll`).
